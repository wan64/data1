...
# 全局路径常量【模块2：增长率看板-保留你Mac本机绝对路径】
FILE_PATH_USER_GROW = "/Users/wang/Desktop/dashboard/注册用户增长率绘图数据3-2.xlsx"
FILE_PATH_CUSTOMER_GROW = "/Users/wang/Desktop/dashboard/客户增长率绘图数据.xlsx"
...
def render_header_logo():
    try:
        ...
        st.markdown("""
        <style>
        .logo-corner {position: absolute;top: 8px;left: 12px;z-index: 999;}
        .logo-corner img {height: 52px;width: auto;object-fit: contain;}
        </style>
...
if board_select == "综合业务":
    ...
    if indicator == "适当性等级":
        ...
        if df is None or len(df) == 0:
            ...
            with right_col:
                st.subheader(f"适当性等级按{time_dim}分布（堆叠柱状图）")
                ...
                st.subheader(f"适当性等级按{time_dim}分布（分组柱状图，剔除未测评）")
    ...
    # ========== 2.机构客户数 ==========
        ...
        if df is None or len(df) == 0:
            ...
            with right_col:
                x = np.arange(len(df))
                st.subheader(f"当月新增机构客户（{time_dim}）")
                ...
                st.subheader(f"累计机构客户（{time_dim}）")
                ...
                st.subheader(f"累计机构用户宽口径（{time_dim}）")
    ...
    # ========== 3.新用户与客户区分 ==========
        ...
        if view_mode == "按时间趋势":
            ...
            if df is None or len(df) == 0:
                ...
                with right_col:
                    st.subheader(f"新用户&客户{time_dim}对比柱状图")
            ...
            if df is None or len(df) == 0:
                ...
                with l_col:
                    ...
                    st.dataframe(df_display,hide_index=True,height=400,use_container_width=True)
                with r_col:
                    st.subheader(f"Top{len(df)}{org_dim}客户排行")
                    fig,ax=plt.subplots(figsize=(10,5))
                    x=np.arange(len(df))
                    vals=df["客户"].values
                    bars=ax.bar(x,vals,color=reds_colors(vals),edgecolor="white",width=0.5)
                    for bar,val in zip(bars,vals):
                        ax.text(bar.get_x()+bar.get_width()/2,bar.get_height()+max(vals)*0.01,f"{val:,}",ha="center",va="bottom",fontsize=7)
                    ax.set_xticks(x)
