# -*- coding: utf-8 -*-
"""
整合版 中信建投期货 双合一数据看板【稳定运行定稿版｜本次新增：Matplotlib图表同行下载按钮功能｜修复Streamlit Cloud Plotly中文方框乱码】
优化清单：
...
5.【重要修复】Matplotlib中文问号问题，全局指定 AlibabaPuHuiTi-3-55-Regular 阿里巴巴普惠体3
6.【新增关键修复】Streamlit Cloud云端Plotly中文□□方框乱码，前端CSS加载本地普惠体+Plotly全局字体强制绑定
...
# ==========【核心修复1：页面顶部注入CSS，加载仓库内阿里巴巴普惠体，解决Plotly前端中文乱码】==========
# 请确认：你的仓库根目录存在字体文件 AlibabaPuHuiTi-3-55-Regular.ttf
font_css = """
<style>
@font-face {
...
/* 全局页面字体兜底 */
html, body, [class*="css"] {
    font-family: "AlibabaPuHuiTi-3-55-Regular", "PingFang SC", "Microsoft YaHei", sans-serif !important;
}
</style>
"""
st.markdown(font_css, unsafe_allow_html=True)

# Plotly全局模板 + 【核心修复2：全局强制Plotly字体】
pio.templates.default = "plotly_white"
# 设置plotly全局默认字体，所有新建Figure自动继承
pio.templates["plotly_white"].layout.font.family = "AlibabaPuHuiTi-3-55-Regular, PingFang SC, Microsoft YaHei, sans-serif"

sns.set_style("whitegrid")

# ========== 【核心修复：Matplotlib中文字体配置 - 使用阿里巴巴普惠体3】 ==========
# 优先加载仓库内阿里巴巴普惠体3，兼容Github云端部署 + Mac本地
        ...
        if df is None or len(df) == 0:
            ...
            with left_col:
                ...
                if df_overall is not None:
                    ...
                    ax_pie.pie(pie_vals, labels=pie_labels, autopct="%1.1f%%", colors=reds_colors(pie_vals), startangle=90, textprops={"fontsize": 9})
                    ax_pie.set_title("各风险等级人数总占比", fontsize=12, fontweight="bold")
                    plt.tight_layout()
            ...
            with right_col:
                ...
                for i, level in enumerate(RISK_LEVELS_5):
                    if level in df.columns:
                        ...
                        bottom += vals
                ax.set_xticks(x)
                ax.set_xticklabels(df[time_col].astype(str), fontsize=8, rotation=45, ha="right")
                ax.set_ylabel("人数")
                ax.legend(title="适当性等级", fontsize=7, loc="upper left")
                ...
                ax2.set_xticks(x2)
                ax2.set_xticklabels(df[time_col].astype(str), fontsize=8, rotation=45, ha="right")
                ax2.set_ylabel("人数")
                ax2.legend(title="适当性等级", fontsize=8)
            ...
            if df_org is None or len(df_org) == 0:
                ...
                ax.set_title(f"Top{len(df_org)}{risk_org_dim}等级分布（{CUTOFF_DATE}）", fontsize=14, weight="bold")
                ax.set_xlabel(risk_org_dim)
                ax.set_ylabel("人数")
                ax.set_xticks(x)
                ax.set_xticklabels(df_org[label_col].astype(str), fontsize=8, rotation=30, ha="right")
                ax.legend(title="适当性等级", fontsize=9)
                ax.spines["top"].set_visible(False)
                ax.spines["right"].set_visible(False)
                plt.tight_layout()
    ...
    # ========== 2.机构客户数 ==========
        ...
        if df is None or len(df) == 0:
            ...
            with right_col:
                ...
                ax3.set_xticks(x)
                ax3.set_xticklabels(df[time_col].astype(str), fontsize=8, rotation=45, ha="right")
                ax3.set_ylabel("数量")
        ...
        if view_mode == "按时间趋势":
            ...
            if df is None or len(df) == 0:
                ...
                with right_col:
                    ...
                    ax.set_xticks(x)
                    ax.set_xticklabels(df[time_col].astype(str), rotation=45, ha="right", fontsize=8)
                    ax.set_ylabel("人数")
            ...
            if df is None or len(df) == 0:
                ...
                with r_col:
                    ...
                    for bar,val in zip(bars,vals):
                        ax.text(bar.get_x()+bar.get_width()/2,bar.get_height()+max(vals)*0.015,f"{val:,}",ha="center",va="bottom",fontsize=7)
                    ax.set_xticks(x)
                    ax.set_xticklabels(df["cap_org_nm"],rotation=45,ha="right",fontsize=7)
                    ax.set_ylabel("客户数")
                    ax.spines["top"].set_visible(False)
                    ax.spines["right"].set_visible(False)
                    plt.tight_layout()
...
# ===================== 分支2：用户增长率可视化看板 =====================
    ...
    if df_raw.empty or df_display.empty:
        ...
        with col_chart:
            ...
            line_y_data = chart_slice[rate_col_name].fillna(0)
            ...
            # =========【关键修复：主动强制当前Figure字体，兜底全局模板失效场景】==========
            fig.update_layout(
                font_family="AlibabaPuHuiTi-3-55-Regular, PingFang SC, Microsoft YaHei, sans-serif",
            ...
            # 【新增：增长趋势实时分析框】
            max_rate = line_y_data.max()
            max_rate_period = x_axis[list(line_y_data).index(max_rate)]
            latest_rate_val = line_y_data.iloc[-1]
            trend_text = "持续走高" if latest_rate_val>line_y_data.iloc[-2] else "阶段性回落"
            trend_analysis = (
        ...
        if select_org_kind == "全公司整体":
            ...
            with bar_col:
                ...
                for bar in bars:
                    h=bar.get_height()
                    ax_bar.text(bar.get_x()+bar.get_width()/2,h+max(top_y)*0.015,f"{int(h):,}",ha="center",va="bottom",weight="bold")
                ax_bar.set_ylabel(f"累计{current_label}")
                ax_bar.tick_params(axis="x",rotation=45)
                ax_bar.grid(axis="y",alpha=0.3)
                ax_bar.set_title("TOP10机构存量降序柱状图")
                plt.tight_layout()
                show_chart_with_download(fig_bar, "TOP10机构存量降序柱状图", "TOP10机构柱状", key_suffix="top10_bar")
                plt.close(fig_bar)
            with pie_col:
                fig_pie, ax_pie = plt.subplots(figsize=(7,5.5))
                wedges, texts, autotexts = ax_pie.pie(top_y,labels=top_x,autopct="%1.1f%%",colors=color_list)
                plt.setp(texts,fontsize=11)
                plt.setp(autotexts,fontsize=11,color="white",weight="bold")
                ax_pie.set_title("TOP10机构内部占比饼图")
                plt.tight_layout()
