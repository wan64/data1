# -*- coding: utf-8 -*-
"""
整合版 中信建投期货 双合一数据看板【稳定运行定稿版｜字体修复完整版】
修复清单：
...
5.【重点修复1】Matplotlib全局字体切换 AlibabaPuHuiTi-3-55-Regular，解决云端中文方框/问号乱码
6.【重点修复2】新增Plotly全局字体强制配置！解决交互式图表中文方框乱码（你之前缺失的关键代码）
适配Mac本机路径、Streamlit云端部署，修复空白/渲染卡顿、内存堆积，结构完整闭环
本机运行命令：终端执行 streamlit run 文件名.py
"""
import matplotlib
# Mac系统绘图后端强制适配，防止画布黑屏
matplotlib.use('Agg')
# ========== 字体加载模块【完整修复区块｜Matplotlib + Plotly双端】 ==========
...
# 字体变量，用于Matplotlib & Plotly共用字体优先级链
FONT_FAMILY_CHAIN = "AlibabaPuHuiTi-3-55-Regular, Noto Sans CJK SC, PingFang SC, Arial Unicode MS, Microsoft YaHei, SimHei, sans-serif"
...
if os.path.exists(FONT_PATH):
    ...
    # 全局rc配置优先普惠体
    matplotlib.rcParams["font.sans-serif"] = [font_name, "Noto Sans CJK SC", "PingFang SC", "Arial Unicode MS", "Microsoft YaHei", "SimHei"]
else:
    # 兜底备选字体，字体文件缺失时启用
    matplotlib.rcParams["font.sans-serif"] = ["Noto Sans CJK SC", "PingFang SC", "Arial Unicode MS", "Microsoft YaHei", "SimHei"]

matplotlib.rcParams["font.family"] = "sans-serif"
matplotlib.rcParams["axes.unicode_minus"] = False

# ===================== Plotly 图表字体修复函数【新增核心！！】 =====================
def fix_plotly_chinese_font(fig):
    """
    所有Plotly图表绘图完成后调用，解决中文方框乱码
    :param fig: plotly Figure 对象
    :return: 修复字体后的figure
    """
    fig.update_layout(
        font=dict(
            family=FONT_FAMILY_CHAIN,
            size=13
        ),
    ...
    if df_raw.empty or df_display.empty:
        ...
        with col_chart:
            ...
            fig.add_trace(go.Bar(
                ...
                text=[f"{int(i):,}" for i in bar_y_data],textposition="inside",textfont={"size":11,"color":"white"},yaxis="y",
            ...
            # ==========【关键调用！修复Plotly中文方框】==========
            fig = fix_plotly_chinese_font(fig)
            # ====================================================
